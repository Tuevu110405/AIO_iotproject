...
class LightGesture:
    ...
    def light_device(self, img, lights):
        ...
        rect_width = width
        white_rect = np.ones((rect_height, rect_width, 3),
                             dtype=np.uint8) * 255

        # Draw a red border around the rectangle
        cv2.rectangle(white_rect, (0, 0),
                      (rect_width, rect_height), (0, 0, 255), 2)
    ...
    def run(self):
        ...
        while cam.isOpened():
            ...
            if len(hand) != 0:
                with torch.no_grad():
                    hand_landmark = torch.from_numpy(
                        np.array(hand[0], dtype=np.float32).flatten()).unsqueeze(0)
                    ...
                    if class_number != -1:
                        ...
                        if self.status_text == "light1":
                            ...
                            if self.light1 and self.light2 and self.light3:
                                ...
                                self.light1 = self.light2 = self.light3 = True
                                if self.device:
                                    self.controller.switch_actuator_1(
                                        self.light1)
                                    time.sleep(0.03)
                                    self.controller.switch_actuator_2(
                                        self.light2)
                                    time.sleep(0.03)
                                    self.controller.switch_actuator_3(
                                        self.light3)
            ...
            img = self.light_simulation(
                img, [self.light1, self.light2, self.light3])

            cv2.putText(img, self.status_text, (5, 20),
...
if __name__ == "__main__":
    ...
    model_path = ".\models\model_30-11 13_55_NeuralNetwork_best"
